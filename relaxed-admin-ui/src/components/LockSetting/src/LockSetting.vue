    ...
    methods: {
      handleSetLock() {
        this.$refs['form'].validate(valid => {
          if (valid) {
            this.$storage.session.setCache('lock_password',this.form.passwd)
            this.handleLock()
          }
        })
      },
      handleLock() {
        const lockPassword=this.$storage.session.getCache('lock_password')
        if (validatenull(lockPassword)) {
          this.box = true
          return
        }
