...
public class CertificateController {
    ...
    @GetMapping("/page" )
    public R<PageResult<CertificatePageVO>> getCertificatePage(
            PageParam pageParam, CertificateQO certificateQO) {
    ...
    public R<Void> updateById(@RequestBody Certificate certificate) {
        return certificateService.updateById(certificate) ?
                R.ok() : R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "修改签章证书表失败");
    }
    ...
    public R<Void> removeById(@PathVariable("certificateId") Integer certificateId) {
        return certificateService.removeById(certificateId) ?
                R.ok() : R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "通过id删除签章证书表失败");
    }
