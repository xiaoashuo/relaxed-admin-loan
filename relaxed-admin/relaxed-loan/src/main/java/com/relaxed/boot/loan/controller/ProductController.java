...
public class ProductController {
	 ...
	 * @return R 通用返回体
	 */
	@Operation(summary = "信托产品下拉列表")
	@GetMapping("/select")
	public R<List<SelectData>> getProductSelectedList() {
		return R.ok(productService.querySelectData());
	}
	...
	@GetMapping("/page")
	public R<PageResult<ProductPageVO>> getProductPage(PageParam pageParam, ProductQO productQO) {
	...
	public R<Void> save(@RequestBody Product product) {
		return productService.save(product) ? R.ok() : R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "新增信托产品表失败");
	}
	...
	public R<Void> updateById(@RequestBody Product product) {
		return productService.updateById(product) ? R.ok()
				: R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "修改信托产品表失败");
	}
	...
	public R<Void> removeById(@PathVariable("productId") Integer productId) {
		return productService.removeById(productId) ? R.ok()
				: R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "通过id删除信托产品表失败");
	}
