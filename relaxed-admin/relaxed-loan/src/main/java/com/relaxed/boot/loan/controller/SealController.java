...
public class SealController {
    ...
    @GetMapping("/page" )
    public R<PageResult<SealPageVO>> getSealPage(
    ...
    public R<Void> updateById(@RequestBody Seal seal) {
        return sealService.updateById(seal) ?
                R.ok() : R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "修改签章图片表失败");
    }
    ...
    public R<Void> removeById(@PathVariable("sealId") Integer sealId) {
        return sealService.removeById(sealId) ?
                R.ok() : R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "通过id删除签章图片表失败");
    }
