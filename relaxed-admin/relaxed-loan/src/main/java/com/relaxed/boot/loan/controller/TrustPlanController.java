...
public class TrustPlanController {
    ...
    @GetMapping("/page" )
    public R<PageResult<TrustPlanPageVO>> getTrustPlanPage(
            PageParam pageParam, TrustPlanQO trustPlanQO) {
    ...
    public R<Void> save(@RequestBody TrustPlan trustPlan) {
        return trustPlanService.save(trustPlan) ?
                R.ok() : R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "新增信托计划配置表失败");
    }
    ...
    public R<Void> updateById(@RequestBody TrustPlan trustPlan) {
        return trustPlanService.updateById(trustPlan) ?
                R.ok() : R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "修改信托计划配置表失败");
    }
    ...
    public R<Void> removeById(@PathVariable("trustPlanId") Integer trustPlanId) {
        return trustPlanService.removeById(trustPlanId) ?
                R.ok() : R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "通过id删除信托计划配置表失败");
    }
