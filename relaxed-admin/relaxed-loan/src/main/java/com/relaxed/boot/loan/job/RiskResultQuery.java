		...
		for (Order order : orderList) {
			...
			try {
				...
				String body = new RiskReqBO().setGuid("D8A0B53B276E4071AF73145212DDE940")
						.setReqId(IdUtil.getSnowflakeNextIdStr()).put("partnerBizNo", partnerBizNo)
						.put("applyAmount", applyAmount).put("createdTime", System.currentTimeMillis())
						.put("realName", orderCustomer.getRealName())
						.put("certificateNo", orderCustomer.getCertificateNo()).toJson();
				...
				if (code == 200) {
					...
					// else if ("REVIEW".equals(riskResult)){}
						...
						String reasonDesc = sb.length() > 0 ? sb.deleteCharAt(sb.length() - 1).toString()
								: sb.toString();
						String reasonCode = reasonCodeBuilder.length() > 0
								? reasonCodeBuilder.deleteCharAt(reasonCodeBuilder.length() - 1).toString()
								: reasonCodeBuilder.toString();
						audit.setReasonCode(reasonCode);
						audit.setReasonDesc(reasonDesc);

					}
					// Map<String, Object> riskRuleMap =
					// data.getByPath("report.activations.hitRuleMap.login_rule",
					// Map.class);
				}
				else if (code == 20003) {
					// 错误map
				...
				log.error(
						LogFormatUtil.format("风控审核查询", "异常", LocalDateTime.now(), StrUtil.format("订单{}", partnerBizNo)),
						e);
			}
