package com.relaxed.boot.risk.controller;

import com.relaxed.boot.risk.model.entity.RiskRuleHistory;
import com.relaxed.boot.risk.model.qo.RiskRuleHistoryQO;
import com.relaxed.boot.risk.model.vo.RiskRuleHistoryVO;
import com.relaxed.boot.risk.service.RiskRuleHistoryService;
import com.relaxed.common.model.domain.PageParam;
import com.relaxed.common.model.domain.PageResult;
import com.relaxed.common.model.result.BaseResultCode;
import com.relaxed.common.model.result.R;
import io.swagger.v3.oas.annotations.Operation;
import io.swagger.v3.oas.annotations.tags.Tag;
import lombok.RequiredArgsConstructor;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

/**
 * <p>
 * 控制器
 * </p>
 *
 * @author Yakir
 * @since 2021-09-11T11:23:02.614
 */
@RequiredArgsConstructor
@RestController
@RequestMapping("rule/history")
@Tag(name = "历史规则控制器")
public class RiskRuleHistoryController {

	private final RiskRuleHistoryService ruleHistoryService;

	/**
	 * 分页查询
	 * @param pageParam {@link PageParam} 分页参数
	 * @param ruleHistoryQO {@link RiskRuleHistoryQO} 查询条件
	 * @return @{code R<PageResult<RuleHistoryVO>>} 通用返回体
	 */
	@GetMapping("/page")
	@Operation(summary = "分页查询", description = "分页查询")
	public R<PageResult<RiskRuleHistoryVO>> page(PageParam pageParam, RiskRuleHistoryQO ruleHistoryQO) {
		return R.ok(ruleHistoryService.selectByPage(pageParam, ruleHistoryQO));
	}

	/**
	 * 新增数据
	 * @param ruleHistory {@link RiskRuleHistory} 数据参数
	 * @return {@code R<?>} 通用返回体
	 */
	@PostMapping
	@Operation(summary = "新增数据", description = "新增数据")
	public R<?> save(@RequestBody RiskRuleHistory ruleHistory) {
		return ruleHistoryService.save(ruleHistory) ? R.ok() : R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "新增数据失败");
	}

	/**
	 * 更新数据
	 * @param ruleHistory {@link RiskRuleHistory} 更新数据
	 * @return {@code R<?>}通用返回体
	 */
	@PutMapping
	@Operation(summary = "更新数据", description = "更新数据")
	public R<?> updateById(@RequestBody RiskRuleHistory ruleHistory) {
		return ruleHistoryService.updateById(ruleHistory) ? R.ok()
				: R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "更新数据失败");
	}

	/**
	 * 根据id删除数据
	 * @param id {@code id} id
	 * @return {@code R<?>} 通用返回体
	 */
	@DeleteMapping("/{id}")
	@Operation(summary = "根据id删除数据", description = "根据id删除数据")
	public R<?> removeById(@PathVariable Long id) {
		return ruleHistoryService.removeById(id) ? R.ok()
				: R.failed(BaseResultCode.UPDATE_DATABASE_ERROR, "根据id删除数据失败");
	}

}
